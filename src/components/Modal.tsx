    ...
    <>
      ...
      <div className="fixed inset-0 flex items-center justify-center z-50 p-4 pointer-events-none">
        <div className="bg-neutral rounded-lg shadow-2xl max-w-md w-full max-h-[90vh] overflow-auto border border-neutral pointer-events-auto">
          {title && (
            <div className="flex justify-between items-center p-4 border-b border-neutral bg-secondary">
              <h2 className="text-xl font-semibold text-secondary">{title}</h2>
              <button
                onClick={onClose}
                className="text-secondary hover:text-negative transition-colors text-2xl leading-none"
              >
                ×
              </button>
            </div>
